...
class TrainingArguments(GRPOConfig):
    ...
    initial_max_turn: Optional[int] = field(default=10)
    rollouts_per_stage: Optional[int] = field(default=1280)
    selected_game: Optional[str] = field(default="gin_rummy")
...
class ActionMaskedGRPOTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: list[dict[str, torch.Tensor | Any]]) -> dict[str, Any]:
        ...
        # Guard: if rewards size isn't divisible by num_generations (can happen during eval
        # when rollout returns fewer results), fall back to per-sample grouping
...
def main():
    ...
    if is_reasoning_tokenizer(tokenizer):
        ...
        # Keep initial_max_turn from config (don't override to 1)
        trainer = GRPOTrainer(
            model=model,
